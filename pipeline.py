...
LUA_SHA1 = get_hash(os.path.join(CWD, 'tiki.lua'))
...
class WgetArgs(object):
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'user':
                wget_args.extend(['--warc-header', 'tiki-user-id: '+item_value])
                wget_args.append('https://tiki.video/@{}/'.format(item_value))
            elif item_type == 'video':
                user, video = item_value.split(':', 1)
                wget_args.extend(['--warc-header', 'tiki-video: '+video])
                wget_args.append('https://tiki.video/@{}/video/{}'.format(user, video))
            else:
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/e/e3/Tiki-video-logo.png" height="50px" title=""/>
        <h2>Tiki <span class="links"><a href="https://tiki.video/">Website</a> &middot; <a href="http://tracker.archiveteam.org/tiki/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/Tiki">Wiki</a></span></h2>
        <p>Archiving Tiki.</p>
    '''
)
